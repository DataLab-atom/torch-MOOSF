...
class MoCo_cls(nn.Module):
    """
    Build a MoCo model with: a query encoder, a key encoder, and a queue
    https://arxiv.org/abs/1911.05722
    """
    def __init__(self, encoder,decoder, dim=128, K=8192, m=0.999, mlp=True,
                 feat_dim=64, feat_t=None, normalize=True, num_classes=1000):
        """
    ...
    def set_distiller(self):
        self.distill = True
        self.encoder_k.load_state_dict(
            torch.load(
                '/home/zz/wenhaibin/cifar/logs/results/cifar100/ce_drw@N_500_ir_100/encoder.pt'))
    ...
    def set_cls_weight(self, cls_weight, base_K=27):
    ...
    def _train(self, logit_q, feat_q, im_k, labels, in_idx):
        """
        Input:
    ...
    def _inference(self, feat_q):
        logit_q, _ = self.decoder_q(feat_q)

        return logit_q

    def forward(self,feat_q=None, im_k=None, labels=None, index=None):
        if self.training:
            return self._train(self._inference(feat_q), feat_q, im_k, labels, index)
        else:
            return self._inference(feat_q)
